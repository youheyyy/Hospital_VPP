...
<div class="max-w-7xl mx-auto w-full space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <div class="relative">
                    ...
                    <input id="searchInput" class="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:border-primary focus:ring-0 rounded-lg transition-all" placeholder="Tìm theo mã phiếu, người tạo..." type="text"/>
                </div>
            </div>
            ...
            <div>
                ...
                <select id="statusFilter" class="w-full py-2.5 px-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:border-primary focus:ring-0 rounded-lg">
            ...
            <div>
                ...
                <select id="dateFilter" class="w-full py-2.5 px-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:border-primary focus:ring-0 rounded-lg">
                    <option value="">Tất cả</option>
    ...
    <!-- Statistics Cards -->
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white dark:bg-slate-900 rounded-xl p-5 shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-shadow">
        ...
        <div class="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-5 shadow-sm border border-amber-200 dark:border-amber-800 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-2">
        ...
        <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-5 shadow-sm border border-green-200 dark:border-green-800 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-2">
        ...
        <div class="bg-red-50 dark:bg-red-900/20 rounded-xl p-5 shadow-sm border border-red-200 dark:border-red-800 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-2">
    ...
    <section class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                        <td class="px-6 py-4">
                            <div class="font-bold {{ $request->status === 'draft' ? 'text-slate-400' : 'text-primary' }}">
                                {{ $request->code }}
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <div class="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold text-xs">
                                    {{ strtoupper(substr($request->user->name ?? 'U', 0, 2)) }}
                                </div>
                                <div>
                                    <div class="font-medium text-sm">{{ $request->user->name ?? 'N/A' }}</div>
                                    <div class="text-xs text-slate-400">{{ $request->department->name ?? 'N/A' }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="font-bold font-mono">{{ number_format($request->total_amount ?? 0) }}</div>
                        ...
                        <td class="px-6 py-4 text-center">
                            @php
                                $statusConfig = [
                                    ...
                                    'completed' => ['bg' => 'bg-blue-100 dark:bg-blue-900/40', 'text' => 'text-blue-700 dark:text-blue-400', 'icon' => 'task_alt', 'label' => 'Hoàn thành'],
                                ];
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                @if($request->status === 'draft')
                                    <a href="{{ route('department.request.edit', $request->id) }}" class="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Chỉnh sửa">
                                        <span class="material-symbols-outlined text-blue-600 dark:text-blue-400">edit</span>
                                    </a>
                                    <button onclick="deleteRequest({{ $request->id }})" class="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Xóa">
                                        <span class="material-symbols-outlined text-red-600 dark:text-red-400">delete</span>
                                    </button>
                                @else
                                    <a href="{{ route('department.request.show', $request->id) }}" class="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Xem chi tiết">
                                    ...
                                    @if(in_array($request->status, ['approved', 'completed']))
                                        <button onclick="printRequest({{ $request->id }})" class="p-2 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg transition-colors" title="In phiếu">
                                            <span class="material-symbols-outlined text-green-600 dark:text-green-400">print</span>
                                        </button>
                                    @endif
                                    @if($request->status === 'rejected')
                                        <button onclick="recreateRequest({{ $request->id }})" class="p-2 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-colors" title="Tạo lại">
                                            <span class="material-symbols-outlined text-amber-600 dark:text-amber-400">refresh</span>
                                        </button>
                                    @endif
                                    @if($request->status === 'pending')
                                        <a href="{{ route('department.request.edit', $request->id) }}" class="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors" title="Chỉnh sửa">
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center justify-center text-slate-400">
                                ...
                                <a href="{{ route('department.request.create') }}" class="mt-4 px-6 py-2.5 bg-primary text-white rounded-lg font-bold hover:bg-sky-700 shadow-lg shadow-primary/20 transition-all flex items-center gap-2">
        ...
        <div class="bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 px-6 py-4 flex items-center justify-between">
            <div class="text-sm text-slate-500">
                Hiển thị <span class="font-bold text-slate-900 dark:text-white">{{ $requests->firstItem() }}-{{ $requests->lastItem() }}</span> 
                trong tổng số <span class="font-bold text-slate-900 dark:text-white">{{ $requests->total() }}</span> phiếu
            </div>
            <div class="flex items-center gap-2">
                {{ $requests->links() }}
            </div>
